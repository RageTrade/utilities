...
const main = async () => {
    ...
    const calculateSizeOfRateArbitrage = async (
        ...
        if ((pFinal - pRage) * fundingSign > 0) {
            const { vQuoteIn, vTokenIn } = await rageTrade.getLiquidityInRange(pRage, pFtx)
    ...
    const arbitrage = async () => {
        ...
        let {maxEthPosition: potentialArbSize} = await calculateSizeOfRateArbitrage(pRage, pFtx, fundingDifference)
